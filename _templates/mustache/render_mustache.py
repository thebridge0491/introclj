...
def render_skeleton(skeleton='skeleton-jvm', ctx=None):
	...
	pat_jar = re.compile('\.jar$')
	for srcC, dstC in copyouts.items():
		if pat_jar.search(srcC):
			import shutil
			shutil.copy(os.path.join(start_dir, srcC),
				os.path.join(CUR_DIR, ctx['name'], dstC))
			continue
...
def parse_cmdopts(args=None):
	...
	opts_parser.add_argument('template', nargs='?', default='skeleton-jvm',
		help='Template path')
