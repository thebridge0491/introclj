...
(defn -main 
    ...
    (let [iniStrm (atom nil) ini-cfg (org.ini4j.Ini.)
		...
		(catch java.io.IOException exc0
			(printf "(exc: %s) Bad env var RSRC_PATH: %s${symbol_escape}n" exc0 rsrc-path)
            (flush)
        ...
        (let [yamlmap (.load yaml @yamlStrm) tup-arr (vector
			...
			(doseq [row tup-arr]
				(printf "config: %s${symbol_escape}n" (get row 0))
				(printf "domain: %s${symbol_escape}n" (get row 1))
				(printf "user1Name: %s${symbol_escape}n${symbol_escape}n" (get row 2))
				))
