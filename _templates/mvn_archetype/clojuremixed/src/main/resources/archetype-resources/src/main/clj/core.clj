...
#set( $symbol_dollar = '$' )
#set( $symbol_escape = '\' )
(ns ^{:doc "Core library"} ${package}.core
    ;{:author "${author}"}
    ;(:require
...
(defn mkstring-init
    ...
    (str
        (reduce 
            (fn [acc el]
                (if (coll? el) 
                    (format "%s%s" acc (mkstring-init "" sep "${symbol_escape}n" el))
...
(defn -main
    ...
    (let [xs (list 0 1 2) ys (list 10 20 30)]
        (printf "(cartesian-prod %s %s): %s${symbol_escape}n" xs ys
            (pr-str (cartesian-prod xs ys)))
        (flush)))

(when (and (.isFile (clojure.java.io/file *file*)) (re-find  ${symbol_pound}"core.clj" *file*))
